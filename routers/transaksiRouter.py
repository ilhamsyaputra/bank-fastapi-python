...
from exceptions.transaksiExceptions import RekeningNotFoundError, LackSaldoError
...
@routes.post('/tarik', status_code=200)
async def transaksiTarikHandler(data: Transaksi, response: Response):
    try:
        transaksiValidator.validate(data)
        transaksiService.addTransaksiTarik(data)

        saldo = transaksiService.getSaldo(data.no_rekening)

        return {
            'success': 'true',
            'message': 'transaksi tarik berhasil',
            'data': {
                'saldo': saldo
            }
        }
    ...
    except LackSaldoError:
        response.status_code = status.HTTP_400_BAD_REQUEST

        return {
            'success': 'false',
            'remark': 'Saldo tidak cukup untuk melakukan penarikan'
        }
